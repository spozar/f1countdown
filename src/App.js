...
function App() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://ergast.com/api/f1/${currentYear}.json`)
      .then((res) => {
        return res.data;
      })
      .then((resdata) => {
  ...
  useEffect(() => {
    ...
    if (raceList) {
      ...
      raceList.forEach((element) => {
        if (element.date + "T" + (element.time ?? "15:00:00Z") > epochTime) {
  ...
  return (
    <MantineProvider theme={{ colorScheme: "dark" }}>
      ...
      >
        ...
        >
          <NextRace raceList={raceList} parsedRaceList={parsedRaceList} />
        </div>
        <div
          style={{
            display: "inline-block",
