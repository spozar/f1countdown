...
import { driverColors, driverTeams } from '../Helpers/DriverColors';
...
function Standings() {
	...
	useEffect(() => {
		...
		setCurrentConstructorStandings(
			JSON.parse(localStorage.getItem('constructorStandings'))
		);
	}, []);
	useEffect(() => {
		const currentYear = new Date().getFullYear();
		...
		if (
			JSON.parse(localStorage.getItem('driverStandings')) !== null
		) {
			setCurrentStandings(
				JSON.parse(localStorage.getItem('driverStandings'))
			);
		}
	}, []);
		...
		return (
			...
			>
				<Text
					...
					size={'2em'}
				>
					Standings for {new Date().getFullYear()}
				</Text>
				<SegmentedControl
					value={value}
					onChange={setValue}
				...
				{value === 'Drivers' ? (
					<div
						style={{ textAlign: 'left', width: 'max(300px, 20vw)' }}
					>
						{currentStandings.map((element, index) => {
							return (
								...
								>
										...
										<div key={index}>
											...
											<motion.div
												...
												style={{
													position: 'relative',
													backgroundColor:
														driverColors[element.Driver.driverId],
													height: '5px',
													paddingRight: '10px',
												...
												animate={{
													width:
														(Number(element.points) /
															Number(currentStandings[0].points)) *
															100 -
														1 +
														'%',
												}}
									...
									<Popover.Dropdown>
										<Text size="sm">
											{driverTeams[element.Driver.driverId]}
										</Text>
									</Popover.Dropdown>
								</Popover>
							);
					...
					<Constructors
						currentConstructorStandings={currentConstructorStandings}
					/>
				)}
			</Card>
		);
